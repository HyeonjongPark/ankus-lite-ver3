...
goods2020 = hs_plus %>% filter(is.na(item) == T, custclass == cu, invoicedate >= "2020-01-01") %>% select(itemname) %>% unique()
goods2020 = goods2020$itemname
goods2020 %>% length

goods2019 = hs_plus %>% filter(is.na(item) == T, custclass == cu, invoicedate >= "2019-01-01" & invoicedate <= "2019-03-01") %>% select(itemname) %>% unique()
goods2019 = goods2019$itemname
goods2019 %>% length

goods2018 = hs_plus %>% filter(is.na(item) == T, custclass == cu, invoicedate >= "2018-01-01" & invoicedate <= "2018-03-01") %>% select(itemname) %>% unique()
goods2018 = goods2018$itemname
goods2018 %>% length

goods2017 = hs_plus %>% filter(is.na(item) == T, custclass == cu, invoicedate >= "2017-01-01" & invoicedate <= "2017-03-01") %>% select(itemname) %>% unique()
goods2017 = goods2017$itemname
goods2017 %>% length

goods2016 = hs_plus %>% filter(is.na(item) == T, custclass == cu, invoicedate >= "2016-01-01" & invoicedate <= "2016-03-01") %>% select(itemname) %>% unique()
goods2016 = goods2016$itemname
goods2016 %>% length

goods2015 = hs_plus %>% filter(is.na(item) == T, custclass == cu, invoicedate >= "2015-01-01" & invoicedate <= "2015-03-01") %>% select(itemname) %>% unique()
...
sam = hs_plus %>% filter(is.na(item) == T, custclass == cu, itemname == current_sell) %>% group_by(invoicedate) 
sam$itemname %>% unique()

arrange(as.data.frame(table(sam$itemname)), Var1)

mape_vec = data.frame(name = current_sell, mape = NA)
mape_vec %>% arrange(name)

leng_vec = data.frame(name = current_sell, len = NA)

setwd(paste0("E:/data-analysis/ankus/ankus-lite-ver2/visualization/인기상품_모델링/홍선/all/"))
getwd()

for(hot_item in 1:(length(current_sell))) {
  
  sgsp270 = hs_plus %>% filter(is.na(item) == T, custclass == cu, itemname == (current_sell[hot_item])) %>% 
...
mape_vec
write.csv(mape_vec, "./mape_info_7.csv")
